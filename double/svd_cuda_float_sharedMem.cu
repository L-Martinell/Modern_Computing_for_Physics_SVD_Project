...
int main()
{
    ...
    MatrixTransposeSHARED<<<gridSize_mm, blockSize_SHARED>>>(m, m, dU_cm, dU);
    MatrixTransposeSHARED<<<gridSize_nn, blockSize_SHARED>>>(n, n, dVT_cm, dVT);
    ...
    MatrixMultiplicationSHARED<<<gridSize2, blockSize2>>>(m, n, m, dU, dSigma, dUSigma);
    CUDA_CHECK_MSG(cudaGetLastError(), "after MatrixMultiplicationKernel (U * Sigma)");
    CUDA_CHECK(cudaDeviceSynchronize());

    MatrixMultiplicationSHARED<<<gridSize2, blockSize2>>>(m, n, n, dUSigma, dVT, dA_rec);
    ...
    // ####### REDUCTION #######
    int reductionThreadsPerBlock = m, reductionGridSize = n;
    ...
    */
    ...
    checkCuda(cudaMemcpy(U, dU, sizeof(float) * m * m, cudaMemcpyDeviceToHost));
